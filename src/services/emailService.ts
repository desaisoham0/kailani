  ...
  try {
    ...
    if (type === 'job' && formData.has('resume')) {
      ...
      if (resumeFile && resumeFile.size > 0) {
        ...
        if (fileSizeMB > 5) {
          throw new Error(
            `Resume file is too large (${fileSizeMB.toFixed(1)}MB). Please use a file smaller than 5MB.`
          );
        }
        ...
        try {
          ...
          throw new Error(
            'Failed to process resume file. Please try with a different file or format.'
          );
        }
      }
    }
    ...
    formData.forEach((value, key) => {
      if (key !== 'resume' && key !== 'type') {
        // Skip resume and type (we handle them separately)
